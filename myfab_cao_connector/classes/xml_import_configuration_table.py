...
class xml_import_configuration_table(models.Model):
    ...
    mf_file_separator = fields.Char(string="Documentary index separator", default="-")
    ...
    def simulation_manager(self, data_dicts_dict, data_elements_ids_list, history_list):
        ...
        """
        self._set_history_list_for_data_list(data_dicts_dict, history_list)

    def _set_history_list_for_data_list(self, data_dicts_dict, history_list, all_data_dicts_dict={}, parent_id=False):
        if not all_data_dicts_dict:
            all_data_dicts_dict = data_dicts_dict
        data_dicts_to_process_dict = self._filter_data_dicts_by_parent_id(data_dicts_dict, parent_id)
        temp_history_list = []
        for data_dict_id in data_dicts_to_process_dict.keys():
            ...
            for key in data_dict:
                if key == "Childrens_list" and data_dict["Childrens_list"]:
                    ...
                    self._set_history_list_for_data_list(
                        self._filter_data_dicts_by_ids(all_data_dicts_dict, children_data_ids_list),
                        children_sim_action_list, all_data_dicts_dict, existing_record
                    )
                    if children_sim_action_list and existing_record:
                        self._add_elements_to_delete_to_children_history_list(children_sim_action_list, existing_record)
                elif key not in ["Childrens_list", "object_relation"]:
                    # Value processing case
                    children_sim_action_list.append(self._get_non_relational_field_sim_action_id(data_dict, key, existing_record, beacon_rc))

            if existing_record:
                if len(existing_record) == 1:
                    # Checking if the import set new values on the existing records
                    if self._is_record_to_update(children_sim_action_list):
                        history_list.append(self._get_sim_action_creation_tuple(
        ...
        for child_creation_tuple in children_history_sorted_list:
            ...
            if not child_beacon_id or child_beacon_id.id != child_creation_dict["mf_beacon_id"]:
                if child_beacon_id and child_beacon_id.relation_openprod_field_id.ttype != "many2one":
                    self._append_delete_element_to_history_list_if_not_checked(
    ...
    def _append_delete_element_to_history_list_if_not_checked(self, history_list, one2many_list, checked_ids_list, beacon_id):
    ...
    def _get_non_relational_field_process_type(self, existing_record, field_setter_id):
        ...
        if (not field_setter_value and field_setter_value not in [False, 0]) or (
            self.env["mf.tools"].are_values_equal_in_same_type(existing_record_field_value, field_setter_value)
        ):
            return "unmodified"
        else:
            return "update"

    def _is_record_to_update(self, children_sim_action_list):
        return self._is_at_least_one_child_modified(children_sim_action_list)

    def _is_at_least_one_child_modified(self, children_sim_action_list):
        for child_sim_action_creation_tuple in children_sim_action_list:
            ...
            if child_sim_action_values_dict["type"] != "unmodified" or self._is_at_least_one_child_modified(
                child_sim_action_values_dict["mf_sim_action_children_ids"]
            ):
                return True
        return False

    def _filter_data_dicts_by_parent_id(self, data_dicts_dict, parent_id):
        filtered_data_dicts_dict = {}
        if parent_id is False:
            for data_dict_id in data_dicts_dict.keys():
                data_dict = data_dicts_dict[data_dict_id]
                if self._is_root_data_dict(data_dict_id, data_dict, data_dicts_dict):
                    filtered_data_dicts_dict[data_dict_id] = data_dict
        else:
            for data_dict_id in data_dicts_dict.keys():
                data_dict = data_dicts_dict[data_dict_id]
                if not self._is_root_data_dict(data_dict_id, data_dict, data_dicts_dict):
                    filtered_data_dicts_dict[data_dict_id] = data_dict
        return filtered_data_dicts_dict

    @staticmethod
    def _filter_data_dicts_by_ids(data_dicts_dict, ids_list):
        filtered_data_dicts_dict = {}
        for data_dict_id in data_dicts_dict.keys():
    ...
    def _is_root_data_dict(self, data_dict_id, data_dict, data_dicts_dict):
        return data_dict["object_relation"].is_main_xml_beacon_relation() and not self._is_id_in_another_data_children_list(
            data_dict_id, data_dicts_dict
        )

    @staticmethod
    def _is_id_in_another_data_children_list(data_id, data_dicts_dict):
        for data_dict_id in data_dicts_dict.keys():
    ...
    def _get_sim_action_creation_tuple(self, process_type, model_name, record_id, beacon_id, children_list, field_setter_id=False):
    ...
    def research_domain_converter(self, domain, data_dict, field_rc, data_dicts_dict=[]):
        ...
        for cond in eval_domain:
            ...
            if field_id.relation and not value and value is not False:
                value = self._get_child_record_id_value_for_relation_field_condition(
    ...
    def _add_parent_id_filter_domain_if_necessary(self, domains_list, record_sim_action_id, parent_id):
    ...
    def _get_child_record_id_value_for_relation_field_condition(self, field_id, data_dict, data_dicts_dict):
        for child_data_id in data_dict["Childrens_list"]:
            if data_dict["Childrens_list"][child_data_id][0].relation_openprod_field_id == field_id:
                ...
                if len(child_record_id) > 1:
                    self._raise_domain_error(child_record_id, child_data_beacon_relation_id)
